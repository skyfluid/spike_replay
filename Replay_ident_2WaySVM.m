...
load(['./AvgSpikePerSec_' def_epoch_name '_bin' int2str(def_binsize) '.mat']);                               % load firing bin map of the epoch
...
save(['./' def_epoch_name '_winArr_' int2str(def_cell_count) 'u_' int2str(def_binsize) 'bs.mat'], 'stampSet', '-v7.3');
...
load(['./PRE_winArr_' int2str(def_cell_count) 'u_' int2str(def_binsize) 'bs.mat']);
...
for iWord=1:length(wordtoVerify)
    ...
    %save(['./score_plot_balance_' def_epoch_name '_' int2str(def_cell_count) 'u_' int2str(def_binsize) 'ms' ], 'score_plot', '-v7.3')

    subplot(1, 3, 1), plot(accusum);
    %plot(cumsum(score_plot{iWord}))
    xlabel('Bin #'); ylabel('Cumulative sum');
    ylim([0 1]);
    title(['PRE ' int2str(def_cell_count) 'u ' int2str(def_binsize) 'ms']);

    %%% TASK
    load(['./TASK_winArr_' int2str(def_cell_count) 'u_' int2str(def_binsize) 'bs.mat']);
    ...
    load(['./POST_winArr_' int2str(def_cell_count) 'u_' int2str(def_binsize) 'bs.mat']);
    ...
    saveas(gcf, ['./' wordtoVerify{iWord} '_accu_' int2str(def_binsize) 'ms.jpg'], 'jpg');
end
%}